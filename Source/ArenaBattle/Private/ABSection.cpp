...
AABSection::AABSection()
{
	...
	{
		ABLOG(Error, TEXT("Error in ABSection.cpp"));
	}

	Trigger = CreateDefaultSubobject<UBoxComponent>(TEXT("TRIGGER")); //스테이지 전체를 감싸는 Box Collision
	...
	{
		ABLOG(Error, TEXT("Failed to load asset"));
	}
	...
	for (FName GateSocket : GateSockets)
	{
		...
		UBoxComponent* NewGateTrigger = CreateDefaultSubobject<UBoxComponent>(*GateSocket.ToString().Append(TEXT("Trigger"))); // 게이트를 감싸는 Box Collision
...
void AABSection::OnKeyNPCDestroyed(AActor* DestroyedActor)
{
	...
	auto ABGameMode = Cast<AABGameMode>(GetWorld()->GetAuthGameMode()); //게임 모드를 가져오는 함수
